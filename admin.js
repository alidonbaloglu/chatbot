    ...
    try {
      ...
      files.forEach((file, index) => {
        ...
        deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          if (!await showConfirm(`"${file.fileName}" dosyasını silmek istediğinize emin misiniz?`)) return;
  ...
  async function loadUserChats() {
    ...
    try {
      ...
      chatArray.forEach(chat => {
        ...
        deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          if (!await showConfirm(`${chat.username} kullanıcısının sohbetini silmek istediğinize emin misiniz?`)) return;
